...
const PreferenceCenter = () => {
    ...
    if (dp_id && df_id) {
      ...
      try {
        const response = await axios.get(
          `https://ngp.adnan-qasim.me/get-preferences?dp_id=${dp_id}&df_id=${df_id}`,
          {
            headers: {
  ...
  const revokeConsent = () => {
    ...
    if (isToggled || dp_id || df_id || consect) {
      axios
        .post(
          `https://ngp.adnan-qasim.me/revoke-consent?dp_id=${dp_id}&df_id=${df_id}&consent_id=${consect}&consent_status=${isToggled}`,
          {
            headers: {
              accept: "application/json",
            },
